 ...
 */
 ...
 public:
     GLWindowContext_ohos(ANativeWindow*, std::unique_ptr<const DisplayParams>);
 ...
 GLWindowContext_ohos::GLWindowContext_ohos(OHNativeWindow* window,
                                            std::unique_ptr<const DisplayParams> params)
         : GLWindowContext(std::move(params))
         , fDisplay(EGL_NO_DISPLAY)
         , fEGLContext(EGL_NO_CONTEXT)
         , fSurfaceOhos(EGL_NO_SURFACE)
 ...
 //    SkDebugf("Extensions: %s", eglQueryString(fDisplay, EGL_EXTENSIONS));
     ...
     fSurfaceOhos = eglCreateWindowSurface(fDisplay, surfaceConfig, fNativeWindow,
                                              surfaceAttribs);
     SkASSERT(EGL_NO_SURFACE != fSurfaceOhos);
 
     SkAssertResult(eglMakeCurrent(fDisplay, fSurfaceOhos, fSurfaceOhos, fEGLContext));
 ...
 void GLWindowContext_ohos::onDestroyContext() {
     if (!fDisplay || !fEGLContext || !fSurfaceOhos) {
         return;
     }
     eglMakeCurrent(fDisplay, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
     SkAssertResult(eglDestroySurface(fDisplay, fSurfaceOhos));
 ...
 void GLWindowContext_ohos::onSwapBuffers() {
     if (fDisplay && fEGLContext && fSurfaceOhos) {
         eglSwapBuffers(fDisplay, fSurfaceOhos);
     }
 }
 ...
 std::unique_ptr<WindowContext> MakeGLForOhos(OHNativeWindow* window,
                                              std::unique_ptr<const DisplayParams> params,
                                              uint32_t width,
                                              uint32_t height) {
     std::unique_ptr<WindowContext> ctx(new GLWindowContext_ohos(window, std::move(params), width, height));
     if (!ctx->isValid()) {
         return nullptr;
     }
     return ctx;
